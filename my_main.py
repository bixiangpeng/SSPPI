...
def read_seq(para):
    pid, dict = para
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/multi_class/seq_data/{pid}.pkl', 'rb') as file:
        seq_features = pkl.load(file)
    dict[pid] = seq_features

def read_graph(para):
    pid, dict = para
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/multi_class/graph_data/{pid}.pkl', 'rb') as file:
        graph_features = pkl.load(file)
    dict[pid] = graph_features
...
if __name__ == '__main__':
    ...
    parse.add_argument("--rst_file", type=str, default='./model_pkl/new_MC/2/results.tsv')
    parse.add_argument("--pkl_path", type=str, default='./model_pkl/new_MC/2/')
