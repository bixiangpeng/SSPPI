...
def collate_dgl(batch): #batch is a list like [(),(),()]
    # g1_list, g2_list, lable_list,p1 ,p2, p1_index, p2_index = list(map(list,zip(*batch)))
    ...
    # return dgl.batch(g1_list), dgl.batch(g2_list), torch.tensor(lable_list), p1, p2, torch.tensor(p1_index,dtype=torch.long), torch.tensor(p2_index,dtype = torch.long)
...
class myDataset(Dataset):
    def __init__(self, contact_dict = None, seq_dict = None, coord_dict = None, graph_dict = None, pns = None,ppi_map_dict = None):
        super(myDataset,self).__init__()
        self.pns = pns
        self.graph_dict = graph_dict
        self.ppi_map_dict = ppi_map_dict
    ...
    def __getitem__(self, index):
        ...
        pid1_ppi_index = self.ppi_map_dict[pid1]
        pid2_ppi_index = self.ppi_map_dict[pid2]
        # g1 , g2 = self.graph_dict[pid1],self.graph_dict[pid2]

        return label,pid1,pid2, pid1_ppi_index,pid2_ppi_index
...
def read_graph(pid,datasetname):
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/{datasetname}/graph_data/{pid}.pkl', 'rb') as file:
...
def read_seq(pid,datasetname):
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/{datasetname}/seq_data/{pid}.pkl', 'rb') as file:
        seq_features = pkl.load(file)
    return  seq_features

class PygDataset(Dataset):
    def __init__(self, dataset_name = 'yeast', seq_dict = None, graph_dict = None, pns = None,ppi_map_dict = None):
    ...
    def __getitem__(self, index):
        ...
        c_size1, features1, edge_index1, rwpe1,edge_index_high1 = self.graph_dict[pid1]
        c_size2, features2, edge_index2, rwpe2, edge_index_high2 = self.graph_dict[pid2]
        seq_data1 = self.seq_dict[pid1]
        seq_data2 = self.seq_dict[pid2]
        ...
        # seq_data1 = padding_seq(pid1, self.dt_name)
        # seq_data2 = padding_seq(pid2, self.dt_name)


        GCNData1 = DATA.Data(x=torch.Tensor(features1), edge_index=torch.LongTensor(edge_index1),
                            y=torch.FloatTensor([label]), rwpe = rwpe1, edge_index_high = torch.LongTensor(edge_index_high1),ppi_id = torch.LongTensor([pid1_ppi_index]), seq_data = seq_data1.unsqueeze(0))
        GCNData1.__setitem__('c_size', torch.LongTensor([c_size1]))

        GCNData2 = DATA.Data(x=torch.Tensor(features2), edge_index=torch.LongTensor(edge_index2),
                            y=torch.FloatTensor([label]), rwpe = rwpe2, edge_index_high = torch.LongTensor(edge_index_high2), ppi_id = torch.LongTensor([pid2_ppi_index]), seq_data = seq_data2.unsqueeze(0))
        GCNData2.__setitem__('c_size', torch.LongTensor([c_size2]))


        # GCNData1.rwpe = compute_pe(GCNData1,20)
        # GCNData2.rwpe = compute_pe(GCNData2, 20)
