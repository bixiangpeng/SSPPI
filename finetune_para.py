...
def main(args):
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/yeast/graph_dict_8_PE_with_2_order.pkl', 'rb') as file:
    ...
    # with open(f'/media/ST-18T/xiangpeng/my_PPI/data/{args.datasetname}/PPI/ppi_graph_GO_in_pyg.pkl', 'rb') as file:
    #     ppi_graph = pkl.load(file)
    # with open(f'/media/ST-18T/xiangpeng/my_PPI/data/{args.datasetname}/PPI/ppi_graph.pkl', 'rb') as file:
    #     ppi_graph = pkl.load(file)
    with open(f'/media/ST-18T/xiangpeng/SSPPI/data/yeast/index_map_dict.pkl', 'rb') as file:
        index_map_dict = pkl.load(file)

    for fold in range(1,6):
        train_pns = []
        with open(f'./data/yeast/5_fold/train3.tsv','r') as fh:
        # with open(f'./data/{args.dataset}/5_fold/train5.tsv', 'r') as fh:
            for line in fh:
                line = line.strip('\n')
                line = line.rstrip('\n')
                words = re.split('  |\t', line)
        ...
        test_pns = []
        with open(f'./data/yeast/5_fold/valid3.tsv', 'r') as fh:
        # with open(f'./data/{args.dataset}/5_fold/valid5.tsv', 'r') as fh:
            for line in fh:
                line = line.strip('\n')
                line = line.rstrip('\n')
                words = re.split('  |\t', line)
        ...
        train_dataset = PygDataset(pns=train_pns, graph_dict=pro_graph_dict, ppi_map_dict=index_map_dict,seq_dict=seq_esm_dict)
        ...
        test_dataset = PygDataset(pns=test_pns, graph_dict=pro_graph_dict, ppi_map_dict=index_map_dict,seq_dict=seq_esm_dict)
        ...
        model_file = f'/media/ST-18T/xiangpeng/SSPPI/model_pkl/new_tuning_para/8.0/fold3{fold}/'
        result_file = f'/media/ST-18T/xiangpeng/SSPPI/model_pkl/new_tuning_para/8.0/fold3{fold}/result.txt'
        # model_file = f'/media/ST-18T/xiangpeng/SSPPI/model_pkl/tuning_para/5x5_3/test{fold}/'
        # result_file = f'/media/ST-18T/xiangpeng/SSPPI/model_pkl/tuning_para/5x5_3/test{fold}/result.txt'
