...
class Attention_block(nn.Module):
    def __init__(self, layer = 1, hidden_dim=64, num_heads=8, dropout=0, batch_first=True, PE = True):
        super(Attention_block, self).__init__()
        self.PE = PE
        if self.PE:
            self.pos_emb = PositionalEncoding(hidden_dim, max_len=45)
        self.encoder = nn.ModuleList([Attention_layer(hidden_dim = hidden_dim, num_heads = num_heads, dropout = dropout, batch_first = batch_first, need_weights = False) for i in range(layer)])
    def forward(self,  inputs_Q, inputs_K, inputs_V, attn_mask, key_padding_mask):
        if self.PE:
            inputs_Q = self.pos_emb(inputs_Q.transpose(0, 1)).transpose(0, 1)
...
class SSPPI(nn.Module):
    ...
    def forward(self,  pro1 , pro2,device):
        ...
        '''## 普通图神经分支'''
        # x1 = self.dropout(self.relu(self.GCN1(x1, edge_index1)))
        # x1 = self.dropout(self.relu(self.GCN2(x1, edge_index1)))
        # x1 = self.relu(self.GCN3(x1, edge_index1))
        #
        # x2 = self.dropout(self.relu(self.GCN1(x2, edge_index2)))
        # x2 = self.dropout(self.relu(self.GCN2(x2, edge_index2)))
        # x2 = self.relu(self.GCN3(x2, edge_index2))

        '''SuperGAT 分支'''
        # x1 = self.dropout(self.elu(self.GCN1(x1, edge_index1)))
        # att_loss = self.GCN1.get_attention_loss()
        # x1 = self.dropout(self.elu(self.GCN2(x1, edge_index1)))
        # att_loss += self.GCN2.get_attention_loss()
        # x1 = self.elu(self.GCN3(x1, edge_index1))
        # att_loss += self.GCN3.get_attention_loss()
        #
        # x2 = self.dropout(self.elu(self.GCN1(x2, edge_index2)))
        # att_loss += self.GCN1.get_attention_loss()
        # x2 = self.dropout(self.elu(self.GCN2(x2, edge_index2)))
        # att_loss += self.GCN2.get_attention_loss()
        # x2 = self.elu(self.GCN3(x2, edge_index2))
        # att_loss += self.GCN3.get_attention_loss()
        ...
        seq1 = self.self_attn1(seq1.transpose(1,2), attn_mask=None, key_padding_mask=None)
        ...
        seq2 = self.self_attn1(seq2.transpose(1,2), attn_mask=None, key_padding_mask=None)
        ...
        #序列池化
        seq1_c = self.mx3(seq1.transpose(1,2))
        seq1_c = seq1_c.squeeze(-1)
        seq2_c = self.mx3(seq2.transpose(1,2))
        seq2_c = seq2_c.squeeze(-1)
        #对比学习
        ...
        seq1_to_2 = self.cross_attn2( seq1_, seq2_, seq2_, attn_mask=None, key_padding_mask=None )
        seq2_to_1 = self.cross_attn2( seq2_, seq1_, seq1_, attn_mask=None, key_padding_mask=None )
        seq1_to_2 = torch.sum( seq1_to_2, 1 )
        seq2_to_1 = torch.sum( seq2_to_1, 1 )
        ...
        hc = self.dropout(hc)
        out = self.out(hc)
        return out, cl_loss, rec_loss #att_loss
        # return rec_loss1
